...
//     name: "Pacman",
//     category: "Arcade",
//     description: "Practice fast decisions and focus.",
//     rewardPoints: 2,
//     mode: "internal",
...
//     name: "Breakout",
//     category: "Arcade",
//     description: "Improve timing, aim, and reaction.",
//     rewardPoints: 2,
//     mode: "internal",
...
//     name: "Flappy",
//     category: "Arcade",
//     description: "Train patience and quick control.",
//     rewardPoints: 2,
//     mode: "internal",
...
//       if (claimed) {
//         localStorage.setItem(`${DAILY_CHALLENGE_PREFIX}${todayKey}`, "claimed");
...
//           Play smart, keep your streak alive, unlock rewards, and come back with
//           better focus.
//         </p>
...
//                   ? `${Math.max(
//                       levelMeta.nextTarget - (streak?.points || 0),
//                       0
...
//             <div>
//               Progress: {todayPlayed.length}/{DAILY_CHALLENGE_GOAL}
//             </div>
//             <div>
//               {dailyClaimed ? "Reward claimed" : "Reward: 8 points"}
//             </div>
//           </div>
...
//                 <div
//                   key={`${item.key}-${item.playedAt}-${index}`}
//                   className="da-card"
//                 >
//                   <h2 style={{ fontSize: "18px" }}>{item.name}</h2>
//                   <p>Played inside website</p>
//                   <p style={{ marginTop: "10px", fontSize: "13px" }}>
...
const allGames = internalGames.map((game) => ({
  ...game,
  mode: "internal"
}));
...
function Gamification() {
  ...
  const selectedGame = useMemo(() => {
    return allGames.find((item) => item.key === selectedGameKey) || null;
  }, [selectedGameKey]);
  ...
  useEffect(() => {
    ...
    if (lastGame && allGames.some((item) => item.key === lastGame)) {
      setSelectedGameKey(lastGame);
    }
          ...
          >
            {achievements.map((badge) => (
              <div
                ...
                style={{ cursor: "default" }}
              >
                <h2 style={{ fontSize: "18px" }}>{badge.title}</h2>
                <p>
                  {badge.unlocked
                    ? "Unlocked"
                    : `${badge.progress}/${badge.goal} progress`}
